...
##### 기수 정렬 : MSD(Most Significant Dist)

- LSD와 반대. 정렬의 기준 선정 방향만을 바꾸어서 기수정렬 진행
- MSD 방식은 점진적으로 정렬이 완성되어가는 방식
- 중간 중간에 정렬이 완료된 데이터는 더 이상 정렬 과정을 진행하지 않아야함

#### 기수 정렬: LSD 기준 구현

- MSD와 LSD의 빅-오는 같음
- LSD 구현이 더 용이. 따라서 LSD를 기준으로 기수 정렬을 구현하는 것이 일반적
- 양의 정수라면 길이에 상관없이 정렬 대상에 포함시키기 위한 간단한 알고리즘
  - NUM으로부터 첫 번째 자리 숫자 추출 `NUM / 1 % 10`
  - NUM으로부터 두 번째 자리 숫자 추출 `NUM / 10 % 10`
  - NUM으로부터 세 번째 자리 숫자 추출 `NUM / 100 % 10`
- LSD 방식에서는 모든 데이터가 정렬의 과정을 동일하게 거치도록 구현
- 반면 MSD 방식에서는 선별해서 거치도록 구현
- 따라서 LSD 방식의 구현이 더 용이할 뿐만 아니라 생각과 달리 성능도 MSD에 비교해서 떨어지지 않음

#### 퀵 정렬 : 성능평가

- 삽입과 추출 연산을 한쌍으로 묶으면, 이 한 쌍의 연산 수행 횟수는 `maxLen * num`
- 따라서 정렬대상의 수가 n이고, 모든 정렬대상의 길이를 l 이라 할때
- 시간 복잡도에 대한 기수 정렬의 빅-오는 `O(ln)`
